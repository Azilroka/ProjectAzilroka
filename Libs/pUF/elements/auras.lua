...
local function UpdateTooltip(self)
	local petOwner, petIndex = unpack(self:GetParent().__owner.petInfo)
	local auraID, _, turnsRemaining, isBuff = C_PetBattles.GetAuraInfo(petOwner, petIndex, self:GetID())
	if not auraID then
		return
	end
	...
	GameTooltip:AddLine(" ")
	_G.PetBattleAbilityTooltip_SetAura(petOwner, petIndex, self:GetID())
...
local function updateIcon(element, petOwner, petIndex, index, offset, isDebuff, visible)
	...
	if not auraID or isBuff == isDebuff then return HIDDEN end
	...
	if(name) then
		...
		if(button.turnsRemaining) then button.turnsRemaining:SetText(turnsRemaining > 0 and turnsRemaining) end
...
local function UpdateAuras(self, _, petOwner, petIndex)
	...
	if(buffs) then
		if(buffs.PreUpdate) then buffs:PreUpdate(petOwner,petIndex) end
		...
		if(buffs.PostUpdate) then buffs:PostUpdate(petOwner, petIndex) end
	end

	local debuffs = self.Debuffs
	if(debuffs) then
		if(debuffs.PreUpdate) then debuffs:PreUpdate(petOwner, petIndex) end
		...
		if(debuffs.PostUpdate) then debuffs:PostUpdate(petOwner, petIndex) end
	end
end
...
local function Enable(self)
	if(self.Buffs or self.Debuffs) then
		self:RegisterEvent("PET_BATTLE_AURA_APPLIED", UpdateAuras)
		self:RegisterEvent("PET_BATTLE_AURA_CANCELED", UpdateAuras)
		self:RegisterEvent("PET_BATTLE_AURA_CHANGED", UpdateAuras)
...
pUF:AddElement('Auras', Update, Enable, Disable, true)
